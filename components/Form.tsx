...
// Type alias for form control children to allow accessing props
...
export const FormControl = React.forwardRef<
  any,
  React.HTMLAttributes<HTMLElement> & { children: AnyElement }
>(({ className, children, ...props }, ref) => {
  const { formItemId, error, formMessageId, onBlur } = useFormField();
  
  const mergedClassName = `${styles.formControl} ${error ? styles.error : ""} ${className || ""} ${children.props.className || ""}`.trim();
  
  const mergedOnBlur = (e: React.FocusEvent) => {
    onBlur();
    children.props.onBlur?.(e);
  };
  
  return React.cloneElement(children, {
    ...props, // Apply FormControl props first
    ...children.props, // Then preserve child's original props
    ref,
    id: formItemId,
